...
bool VoronoiDiagramGenerator::
generateVoronoi(double *xValues, double *yValues, int numPoints,
                double minX, double maxX, double minY, double maxY, double minDist)
{
	cleanup();
	cleanupEdges();
	int i;
...
void VoronoiDiagramGenerator::pushGraphEdge(double x1, double y1, double x2, double y2, Site* sites[])
{
	GraphEdge* newEdge = new GraphEdge;
	newEdge->next = allEdges;
  newEdge->siteIncides[0] = sites[0]->sitenbr;
  newEdge->siteIncides[1] = sites[1]->sitenbr;
	allEdges = newEdge;
	newEdge->x1 = x1;
	newEdge->y1 = y1;
	newEdge->x2 = x2;
...
void VoronoiDiagramGenerator::line(double x1, double y1, double x2, double y2, Site* sites[])
{	
	pushGraphEdge(x1,y1,x2,y2, sites);
